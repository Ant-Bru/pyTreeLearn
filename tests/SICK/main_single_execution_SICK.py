...
def main(args):
    ...
    logger = set_main_logger_settings(log_dir, 'main_SICK_'+str(args.h_size)+"_"+args.cell_type+"_"+str(args.pos_stationarity)+"_"+str(args.freeze))
    ...
    # load the data
    trainset, devset, testset = load_sick_dataset(args.wetype)

    # create the model
    model = create_sick_model(trainset.we_size,
                              args.h_size,
    ...
    best_model, best_dev_metrics, *others = train_and_validate(model, sick_extract_batch_data, sick_loss_function, optimizer, trainset, devset, device,
                                                      metrics_class=[Pearson_sick, MSE_sick],
                                                      batch_size=args.batch_size,
                                                      n_epochs=args.epochs, early_stopping_patience=args.early_stopping)

    # test results
    return test(best_model, sick_extract_batch_data,  testset, device,
         metrics_class=[Pearson_sick, MSE_sick],
         batch_size=args.batch_size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save', default='tests/SICK/checkpoints/')
    parser.add_argument('--expname', default='test_SICK_')
